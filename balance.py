...
def build_balance(settings: Settings) -> tuple[pd.DataFrame, object]:
    ...
    df = read_table(settings.input_dir, "FAGLFLEXT", fields)
    ...
    df["期初余额"] = df["HSLVT"]
    movement_cols = [f"HSL{i:02d}" for i in range(1, 7)]
    ...
    result = df.groupby(["RCLNT", "RBUKRS", "RACCT"], as_index=False)[["期初余额", "借方发生额", "贷方发生额", "期间净发生额"]].sum()
    result["期末余额"] = result["期初余额"] + result["期间净发生额"]
    amount_cols = ["期初余额", "借方发生额", "贷方发生额", "期间净发生额", "期末余额"]
    ...
    cols = ["公司代码", "公司名称", "总账科目", "科目名称", "本位币", "期初余额", "借方发生额", "贷方发生额", "期间净发生额", "期末余额"]
    result = result[cols].sort_values(["公司代码", "总账科目"])
    path = settings.output_dir / "科目余额表_2026H1.csv"
    result.to_csv(path, index=False, encoding="utf-8-sig")
    return result, path


def validate_journal_to_balance(settings: Settings) -> tuple[pd.DataFrame, object]:
    journal_path = settings.output_dir / "序时账期间汇总_2026H1.csv"
    ...
    path = settings.output_dir / "序时账与余额表核对_2026H1.csv"
    check.to_csv(path, index=False, encoding="utf-8-sig")

    bcf_path = settings.output_dir / "余额结转BCF汇总_2026H1.csv"
    if bcf_path.exists():
        ...
        opening = opening.merge(
            balance[["公司代码", "总账科目", "期初余额"]], how="outer",
            on=["公司代码", "总账科目"], indicator=True,
        )
        for col in ["BCF记录数", "ACDOCA余额结转", "期初余额"]:
            opening[col] = pd.to_numeric(opening[col], errors="coerce").fillna(0.0)
        opening["差异"] = (opening["ACDOCA余额结转"] - opening["期初余额"]).round(2)
        opening["核对结果"] = opening["差异"].abs().le(0.01).map({True: "一致", False: "差异"})
        opening.to_csv(settings.output_dir / "ACDOCA余额结转与余额表期初核对_2026H1.csv", index=False, encoding="utf-8-sig")
    return check, path
